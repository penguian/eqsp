"""
Figure 3.6: Maximum diameters of EQ(4, N) (log-log scale).

For each partition EQ(4, N) for N from 1 to 100,000, plot the
diameter bound coefficient N^(1/dim) * diam_bound (red dots) and the
vertex diameter coefficient (blue +), as in Figure 3.6 of the thesis.
...
def main():
    ...
    ax.set_xlabel("N")
    ax.set_ylabel(r"$N^{1/4} \cdot \mathrm{diam}$")
    ax.set_title(r"Maximum diameters of $\mathrm{EQ}(4, N)$ (log-log scale)")
    ax.legend()
    plt.tight_layout()

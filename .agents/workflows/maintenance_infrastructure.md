...
When modifying scripts in `release/`, `validation/`, or project-wide documentation (Maintenance/User Guides), you MUST perform the following checks before concluding the session.
...
4. **Synchronize Records**:
    - Update the **Maintenance Guide** (`doc/maintenance_guide.md`) if the "Defense in Depth" strategy or tool inventory changed.
    - Update the version-specific **Release Notes** (e.g., `doc/maintainer/release_notes.md`).

"""
Figure 4.6: Packing density of EQP(3, N) (semi-log scale).

For N from 1 to 20,000, plot the packing density against N on a
semi-log scale, as in Figure 4.6 of the thesis.
...
def main():
    ...
    ax.semilogx(
        ...
        "b+",
        markersize=2,
        label=r"$\mathrm{packing density} \ \mathrm{EQP}(3)$",
    )
    ax.axhline(
        y=sc_density,
        color="r",
    ...
    fig.text(
        0.5,
        0.02,
        r"Figure 4.6: Packing density of $\mathrm{EQP}(3,\mathcal{N})$ "
        r"(semi-log scale)",
        ha="center",
        fontsize=10,
    )

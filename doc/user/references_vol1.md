# References

This chapter lists all scientific works cited throughout the **User Guide (Volume 1)** and the **API Appendix**.

---

(arr12)=
- **[Arr12]** Arrigo, N., Albert, L. P., Mickelson, P. G., and Barker, M. S. (2012). *"Quantitative visualization of biological data in Google Earth using R2G2"*, Molecular Ecology Resources, 12(6), 1177-1179.
(ben21)=
- **[Ben21]** Benedikter, N., Nam, P. T., Porta, M., Schlein, B., and Seiringer, R. (2021). *"Correlation energy of a weakly interacting Fermi gas"*, Inventiones mathematicae, 225(3), 885-979.
(chu09)=
- **[Chu09]** Chu, V. B., Lipfert, J., Bai, Y., Pande, V. S., Doniach, S., and Herschlag, D. (2009). *"Do conformational biases of simple helical junctions influence RNA folding stability and specificity?"*, RNA, 15(12), 2195-2205.
(dah78)=
- **[Dah78]** Dahlberg, B. E. J. (1978). *"On the distribution of Fekete points"*, Duke Math. J. 45 (1978), no. 3, pp. 537--542.
(das20)=
- **[Das20]** Das, S. and Maharatna, K. (2020). *"An automated toolchain for quantitative characterisation of structural connectome from MRI based on non-anatomical cortical parcellation"*, IEEE EMBC, 5653-5656.
(fau08)=
- **[Fau08]** Fauchereau, N., Pegram, G. G. S., and Sinclair, S. (2008). *"Empirical Mode Decomposition on the sphere: application to the spatial scales of surface temperature variations"*, Hydrology and Earth System Sciences, 12(3), 933-941.
(ham21)=
- **[Ham21]** Hammer, M. D., Cox, G. A., Brown, W. J., Beggan, C. D., and Finlay, C. C. (2021). *"Geomagnetic Virtual Observatories: monitoring geomagnetic secular variation with the Swarm satellites"*, Earth, Planets and Space, 73(1), 1-22.
(kui98)=
- **[Kui98]** Kuijlaars, A. B. J. and Saff, E. B. (1998). *"Asymptotics for minimal discrete energy on the sphere"*, Transactions of the American Mathematical Society, v. 350 no. 2, pp. 523--538.
(kui07)=
- **[Kui07]** Kuijlaars, A. B. J., Saff, E. B., and Sun, X. (2007). *"On separation of minimal Riesz energy points on spheres in Euclidean spaces"*, Journal of Computational and Applied Mathematics, 199(1), 172-180.
(laz20)=
- **[Laz20]** Lazarus, C., Weiss, P., El Gueddari, L., et al. (2020). *"3D variable-density SPARKLING trajectories for high-resolution T2*-weighted magnetic resonance imaging"*, NMR in Biomedicine, 33(9), e4349.
(leo06)=
- **[Leo06]** Leopardi, P. (2006). *"A partition of the unit sphere into regions of equal area and small diameter"*, Electronic Transactions on Numerical Analysis, Volume 25, pp. 309-327.
(leo07)=
- **[Leo07]** Leopardi, P. (2007). *"Distributing points on the sphere: Partitions, separation, quadrature and energy"*, PhD thesis, UNSW.
(leo09)=
- **[Leo09]** Leopardi, P. (2009). *"Diameter bounds for equal area partitions of the unit sphere"*, Electronic Transactions on Numerical Analysis, Volume 35, pp. 1-16.
(leo24jas)=
- **[Leo24-JAS]** Leopardi, P. (2024). *"The applicability of equal area partitions of the unit sphere"*, Journal of Approximation Software, 1(2). DOI: 10.13135/jas.10248.
(mat21)=
- **[Mat21]** Matsuyama, I., Keane, J. T., Trinh, A., Beuthe, M., and Watters, T. R. (2021). *"Global tectonic patterns of the Moon"*, Icarus, 358, 114202.
(moz15)=
- **[Moz15]** Mozdzynski, G., Hamrud, M., and Wedi, N. (2015). *"A partitioned global address space implementation of the European Centre for Medium Range Weather Forecasts Integrated Forecasting System"*, Int. J. High Perform. Comput. Appl., 29(3), 261-273.
(pfa20)=
- **[Pfa20]** Pfaff, F., Li, K., and Hanebeck, U. D. (2020). *"A Hyperhemispherical Grid Filter for Orientation Estimation"*, Proc. 23rd Int. Conf. Information Fusion. DOI: 10.23919/FUSION45008.2020.9190611.
(rak94)=
- **[Rak94]** Rakhmanov, E. A., Saff, E. B., and Zhou, Y. M. (1994). *"Minimal discrete energy on the sphere"*, Mathematics Research Letters, 1, pp. 647--662.
(saf97)=
- **[Saf97]** Saff, E. B. and Kuijlaars, A. B. J. (1997). *"Distributing many points on a sphere"*, Mathematical Intelligencer, v19 no1, pp. 5--11.
(sab21)=
- **[Sab21]** Sabiston, T., Lee-Sullivan, P., and Inal, K. (2021). *"Artificial intelligence approach for increasing the fidelity of the second order fibre orientation tensor"*, Composite Structures, 275, 114393.
(wer18)=
- **[Wer18]** Werner, J. P., Divine, D. V., Charpentier Ljungqvist, F., Nilsen, T., and Francus, P. (2018). *"Spatio-temporal variability of Arctic summer temperatures over the past 2 millennia"*, Climate of the Past, 14(4), 527-557.
